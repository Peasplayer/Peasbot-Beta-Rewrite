...
module.exports = {
  ...
  run: async (client, message, args, config, db) => {
    ...
    if(client.player.getQueue(message.guild).repeatMode == 0) {
      message.channel.send("🔁 Loop enabled!")
    } else {
      message.channel.send("🔁 Loop disabled!")
    }
    client.player.getQueue(message.guild).setRepeatMode(client.player.getQueue(message.guild).repeatMode == 0 ? 2 : 0)
  }
}
